...
import { Calendar, TrendingUp, TrendingDown, DollarSign, Plus, Minus, BarChart3, History } from 'lucide-react';

const CryptoTracker = () => {
  // State variables for managing application data and UI
  const [selectedCrypto, setSelectedCrypto] = useState('BTC'); // Currently selected cryptocurrency (e.g., BTC, ETH)
  const [currency, setCurrency] = useState('USD'); // Currently selected display currency (e.g., USD, ARS)
  const [currentView, setCurrentView] = useState('tracker'); // Current view: 'tracker' or 'history'
  const [transactions, setTransactions] = useState([]); // Array to store all recorded transactions
  const [newTransaction, setNewTransaction] = useState({ // State for the new transaction form
  ...
  const [cryptoDetails, setCryptoDetails] = useState({ // Stores real-time price and change data for cryptocurrencies
  ...
  const [chartData, setChartData] = useState([]); // Data for the profit line chart
  ...
  // useEffect hook to fetch prices on component mount and then every 60 seconds
  useEffect(() => {
  ...
  // useEffect hook to update the new transaction's price when selected crypto or its price changes
  useEffect(() => {
  ...
  // useEffect hook to calculate and update chart data whenever transactions change
  useEffect(() => {
